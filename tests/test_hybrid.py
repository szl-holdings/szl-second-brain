...
def _write_corpus(path: Path) -> dict[str, str]:
    ...
    for node_id, text in texts.items():
        rows.append({"id": node_id, "source": sources[node_id], "title": text.title(),
...
def test_bm25_only_mode_is_honest_and_handles_only(corpus):
    path, _texts = corpus
    result = HybridSecondBrain(SecondBrainIndex(path)).context("formula authority", k=2)
    ...
    assert all(set(handle) == {"nodeId", "nodeKind", "label", "note"} for handle in result["handles"])
...
def test_sparse_dense_rrf_and_reranker_are_bound_to_known_nodes(corpus):
    ...
    def dense(query: str, k: int):
        ...
        return [{"node_id": "n4", "score": 0.99}, {"node_id": "n1", "score": 0.80}]
    def rerank(query: str, candidates, k: int):
        assert query
        assert all("content" not in item for item in candidates)
        return ["n1", "n4"]
    result = HybridSecondBrain(SecondBrainIndex(path), dense_provider=dense, reranker=rerank,
                              per_source_limit=3).context("formula authority anatomy", k=2)
...
def test_unknown_dense_node_fails_closed_when_fallback_disabled(corpus):
    ...
    with pytest.raises(RetrievalBoundaryError, match="dense provider unavailable"):
        HybridSecondBrain(SecondBrainIndex(path), dense_provider=poisoned_dense,
                          allow_sparse_fallback=False).context("formula", k=2)


def test_dense_failure_falls_back_with_explicit_label(corpus):
    path, _texts = corpus
    def failed_dense(query: str, k: int):
        # An adapter must explicitly distinguish outage from integrity/ACL bugs.
        raise ProviderUnavailable("provider offline")
    result = HybridSecondBrain(SecondBrainIndex(path), dense_provider=failed_dense,
                              allow_sparse_fallback=True).context("formula", k=2)
    assert result["ready"] is True
    assert result["ranking_receipt"]["mode"] == "BM25_FALLBACK_DENSE_UNAVAILABLE"
    assert result["ranking_receipt"]["dense"] == "UNAVAILABLE"
    assert result["ranking_receipt"]["dense_error_type"] == "ProviderUnavailable"
...
def test_reranker_cannot_introduce_nodes(corpus):
    ...
    def rerank(query: str, candidates, k: int):
        return ["private:node"]
    # Poisoned output now fails the entire request; it is not a service outage.
    with pytest.raises(RetrievalBoundaryError, match="unknown node IDs"):
        HybridSecondBrain(SecondBrainIndex(path), reranker=rerank).context("formula", k=2)


def test_empty_query_abstains_without_providers(corpus):
    path, _texts = corpus
    result = HybridSecondBrain(SecondBrainIndex(path)).context("", k=2)
...
def test_authorized_hydration_verifies_scope_and_content_digest(corpus):
    ...
    result = hydrator.hydrate([{"nodeId": "n1", "source": "lean", "sha256": digest}],
                             principal_id="principal-1", tenant_id="tenant-1", policy_revision="a" * 40)
...
def test_hydration_denial_is_batch_fail_closed(corpus):
    ...
    def authorize(principal, tenant, policy, node_id, source):
        return node_id != "n2"
    hydrator = AuthorizedHydrator(authorize, path=path)
    handles = [{"nodeId": node_id, "source": source,
    ...
    with pytest.raises(RetrievalBoundaryError, match="access denied"):
        hydrator.hydrate(handles, principal_id="principal-1", tenant_id="tenant-1",
...
def test_hydration_rejects_tampered_handle_digest(corpus):
    ...
    with pytest.raises(RetrievalBoundaryError, match="digest mismatch"):
        hydrator.hydrate([{"nodeId": "n1", "source": "lean", "sha256": "0" * 64}],
                         principal_id="principal-1", tenant_id="tenant-1", policy_revision="c" * 40)


def test_hydrator_refuses_corrupt_corpus(tmp_path: Path):
    path = tmp_path / "corrupt.jsonl"
    path.write_text(json.dumps({"id": "n1", "source": "lean", "title": "bad", "text": "actual",
                                "sha256": "0" * 64}) + "\n", encoding="utf-8")
